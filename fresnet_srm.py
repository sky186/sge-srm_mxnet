...
def residual_unit_v3(data, num_filter, stride, dim_match, name, bottle_neck, **kwargs):
    ### units = [3, 4, 14, 3],res50,use to sge reshape transport is 3 4 14 3
    ...
    use_srm = kwargs.get('version_srm', 1)  #default 1 ,the input is 1,me add
    ...
    if bottle_neck:
        ...
        if use_se:
          ...
          # data_shape = (32, 3, 112, 112)
          # print(body.infer_shape(data=data_shape)[1][0])
        ...
        if use_srm:
          ...
          #print (name)
          groups=64
          batch = config.per_batch_size
          data_shape = (batch, 3, 112, 112)
          shape_size = bn3.infer_shape(data=data_shape)[1][0] #[bchw]
          
          channel_mean = mx.sym.mean(mx.sym.reshape(bn3, shape=(0,0,-1)),axis=2,keepdims=True)

          sub = mx.sym.broadcast_sub(mx.sym.reshape(bn3, shape=(0,0,-1)),channel_mean)
          var =mx.sym.mean((sub**2),axis=2,keepdims=True) + 1e-5
          channel_std = mx.symbol.sqrt(var)

          srm_weight = mx.symbol.Variable(name+"_srm_weight"+name, shape=(shape_size[1],2),lr_mult=1.0,wd_mult=1.0,init=mx.init.Normal(0.001))#(c,2)
          t = mx.symbol.concat(channel_mean,channel_std,dim=2) # B x C x 2
          cfc = mx.sym.broadcast_mul(t,srm_weight)  #bc2
          z=mx.sym.sum(cfc,axis=2)  #bc  
          ### expand    bc11
          z=mx.sym.expand_dims(data=z,axis=-1)
          z=mx.sym.expand_dims(data=z,axis=-1)
          bn_srm=mx.sym.BatchNorm(data=z, fix_gamma=False, eps=2e-5, momentum=bn_mom, name=name + '_bn_srm')
          g=mx.symbol.Activation(data=bn_srm, act_type='sigmoid', name=name+"_srm_sigmoid")
          # bchw *bc11
          bn3 = mx.symbol.broadcast_mul(bn3, g)

          #print(bn3.infer_shape(data=data_shape)[1][0])
...
def residual_unit_v1_L(data, num_filter, stride, dim_match, name, bottle_neck, **kwargs):
    ...
    #print('in unit1')
    if bottle_neck:##False  is 3*3 3*3 not 1*1
        ...
        if use_se:  #config config.net_se = 0 not to use se 
          #se begin
...
def residual_unit(data, num_filter, stride, dim_match, name, bottle_neck, **kwargs):
  uv = kwargs.get('version_unit', 3)  #default is 3 and tansfer is no,  'version_input': config.net_input,is 3
  version_input = kwargs.get('version_input', 1)
  if uv==1:
    if version_input==0:
      return residual_unit_v1(data, num_filter, stride, dim_match, name, bottle_neck, **kwargs)
    else:
      return residual_unit_v1_L(data, num_filter, stride, dim_match, name, bottle_neck, **kwargs) #select this 
  if uv==3: ##select
...
def resnet(units, num_stages, filter_list, num_classes, bottle_neck):
    ...
    if version_input==0:
      ...
      body = mx.sym.BatchNorm(data=body, fix_gamma=False, eps=2e-5, momentum=bn_mom, name='bn0')
      body = Act(data=body, act_type=act_type, name='relu0')
    else:  #version_input==1    excute
      data = mx.sym.identity(data=data, name='id')
      data = data-127.5
      data = data*0.0078125
      body = data
      body = Conv(data=body, num_filter=filter_list[0], kernel=(3,3), stride=(1,1), pad=(1, 1),  ##same convolution311,112*112
                                no_bias=True, name="conv0", workspace=workspace)  #this is L  7*7 to 3*3
      body = mx.sym.BatchNorm(data=body, fix_gamma=False, eps=2e-5, momentum=bn_mom, name='bn0')
      body = Act(data=body, act_type=act_type, name='relu0')
    ...
    for i in range(num_stages):  #4
      ...
      ### # shortcut have conv and bn  , reduce size 321  112 to 56

      body = residual_unit(body, filter_list[i+1], (2, 2), False,  ##filter_list = [64, 64, 128, 256, 512],False input output dinmension different
        name='stage%d_unit%d' % (i + 1, 1), bottle_neck=bottle_neck, **kwargs)  #
      ###  ## # shortcut no conv and bn  no reduce,  56 to 56 ,repeat 3 times
      
      for j in range(units[i]-1):      ##  #units = [3, 13, 30, 3]  #3 residual_unit
    ...
    fc1 = symbol_utils.get_fc1(body, num_classes, fc_type) #c=512 dimension
    return fc1
