...
def residual_unit_v3(data, num_filter, stride, dim_match, name, bottle_neck, **kwargs):
    ...
    use_sge = kwargs.get('version_sge', 1)  #default 1 ,the input is 1,me add
    ...
    if bottle_neck:
        ...
        if use_sge:
          ...
          body = mx.sym.BatchNorm(data=body, fix_gamma=False, eps=2e-5, momentum=bn_mom, name=name + '_sge_bn')
          body = mx.symbol.Activation(data=body, act_type='sigmoid', name=name + "_sge_sigmoid")
          body = mx.symbol.broadcast_mul(x, body)
          # print("^^^^^^^^^^^^^^^^^^^")
          # print(body.infer_shape(data=data_shape)[1][0])
          b = mx.sym.reshape(body, shape=(-4, -1,groups,0, 0, 0))
          bn3 = mx.sym.reshape(b, shape=(0, -3, 0, 0))
          # b=mx.symbol.Reshape(data=body, shape=(0, -4, groups, -1, 0, 0))
          # b = mx.symbol.Reshape(data=body, shape=(-4, 0, 0))

          # print(body.infer_shape(data=data_shape)[1][0])
          # print(b.infer_shape(data=data_shape)[1][0])
          # print("##########################################")
        ...
        if use_sge:
          ...
          batch = config.per_batch_size
          data_shape = (batch, 3, 112, 112)
          shape_size = bn3.infer_shape(data=data_shape)[1][0]
